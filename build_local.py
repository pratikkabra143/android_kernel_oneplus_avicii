...
out_dir="$HOME/Nord_Kernel/out_$(date +"%d%m%Y_%H%M")"
mkdir -p "$out_dir"
cp arch/arm64/configs/avicii_defconfig arch/arm64/configs/avicii_defconfig.bak
trap 'mv -f arch/arm64/configs/avicii_defconfig.bak arch/arm64/configs/avicii_defconfig 2>/dev/null' EXIT
...
if command -v ccache &>/dev/null; then
    ...
    echo -e ${LGR} "⚠️ ccache not found, building without cache (install ccache for faster rebuilds)"${NC}
    export CC="clang"
fi
...
completion() {
  ...
  if [[ -f ${ZIMAGE} && -f ${DTBOIMAGE} ]]; then
    ...
    cp modules/kmu-nsk/module.prop modules/kmu-nsk/module.prop.bak
    ...
    mv $kf/$zip_name $out_dir/$zip_name
    mv -f anykernel.sh.bak anykernel.sh
    mv -f modules/kmu-nsk/module.prop.bak modules/kmu-nsk/module.prop
    echo -e ${LGR} "📦 Flashable zip created at $out_dir/$zip_name ✅"${NC}
    md5sum "$out_dir/$zip_name" > $out_dir/checksums.txt
    CHANGELOG_TEXT="${CHANGELOG:-* No Changelog provided}"
    echo -e "NeverSettle Kernel $version Changelog ($build_date)\\n=========================================\\n$CHANGELOG_TEXT" > $out_dir/changelog.txt
    echo -e ${LGR} "📝 Generated changelog.txt and checksums.txt ✅"${NC}
    echo -e ${LGR} "🧑‍💼 Downloading KernelSU-Next manager..."${NC}
    curl -sL ${ksu_apk} > $out_dir/${ksu_apk_name}
    END=$(date +"%s")
    DIFF=$(($END - $START))
